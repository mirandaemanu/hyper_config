...
fix_jump_connection() {
    ...
    if  ping -c 4 $jump > /dev/null 2>&1 ; then
        ...
        sudo bash -c "sed -i '/# generateHosts = false/a 10.25.73.241   jump1.pro1.eigbox.com/a 10.25.73.242   jump2.pro1.eigbox.com\a 185.199.108.133   raw.githubusercontent.com'"
        echo -e "${cor_verde}Conexão bem sucedida ${reset}"
    fi
}
